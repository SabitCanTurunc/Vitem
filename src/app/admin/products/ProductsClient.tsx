...
export default function ProductsClient({ prods, categories }: { prods: Product[]; categories: Category[] }) {
  ...
  async function handleTranslate() {
    ...
    const [tName, tDesc, tShort] = await Promise.all([
      translate(nameVal), translate(descVal), translate(shortVal),
    ]);
    setNameEnVal(tName); setDescEnVal(tDesc); setShortEnVal(tShort);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-serif font-light text-vitem-900">Ürünler</h1>
          <p className="text-sm text-vitem-500 mt-1">{prods.length} ürün</p>
        </div>
      ...
      <div className="bg-white border border-vitem-200 overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-vitem-50 border-b border-vitem-200 text-vitem-500 uppercase tracking-wider text-xs">
            <tr>
              <th className="px-6 py-4 font-medium">Ad (TR / EN)</th>
          ...
          <tbody className="divide-y divide-vitem-100">
            {prods.length === 0 ? (
              <tr><td colSpan={4} className="px-6 py-12 text-center text-vitem-400 text-sm">Henüz ürün yok.</td></tr>
            ) : prods.map((prod) => (
              <tr key={prod.id} className="hover:bg-vitem-50/50 transition-colors">
                <td className="px-6 py-4">
                  <p className="font-medium text-vitem-900">{prod.name}</p>
                  <p className="text-xs text-vitem-400 mt-0.5">{prod.nameEn || <span className="text-amber-500">⚠ Çeviri yok</span>}</p>
                </td>
                <td className="px-6 py-4 text-vitem-600">{prod.categoryName || "—"}</td>
                <td className="px-6 py-4">
                  {prod.isFeatured
                    ? <span className="bg-green-100 text-green-700 px-2 py-0.5 text-[10px] uppercase tracking-wider">Öne Çıkan</span>
                    : <span className="bg-vitem-100 text-vitem-500 px-2 py-0.5 text-[10px] uppercase tracking-wider">Standart</span>}
                </td>
                <td className="px-6 py-4 text-right">
                  <button onClick={() => openEdit(prod)} className="p-2 text-vitem-400 hover:text-blue-600 transition-colors" title="Düzenle">
                    <Edit className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(prod.id)} disabled={deletingId === prod.id} className="p-2 text-vitem-400 hover:text-red-600 transition-colors disabled:opacity-40" title="Sil">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-8 py-5 border-b border-vitem-100">
                <h2 className="text-xl font-serif font-light text-vitem-900">{modalMode === "edit" ? "Ürün Düzenle" : "Yeni Ürün"}</h2>
                <div className="flex items-center gap-3">
                  <button type="button" onClick={handleTranslate} disabled={isTranslating || !nameVal.trim()}
                    className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 disabled:opacity-40 border border-blue-200 px-3 py-1.5 hover:bg-blue-50 transition-colors">
              ...
              <form action={handleSubmit} className="px-8 py-6 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <F label="Ad (TR) *" name="name" value={nameVal} onChange={setNameVal} required />
                  <F label="Ad (EN)" name="nameEn" value={nameEnVal} onChange={setNameEnVal} />
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <F label="Kısa Açıklama (TR)" name="shortDescription" value={shortVal} onChange={setShortVal} />
                  <F label="Kısa Açıklama (EN)" name="shortDescriptionEn" value={shortEnVal} onChange={setShortEnVal} />
                </div>
                <div className="grid grid-cols-2 gap-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    <label className="text-xs uppercase tracking-widest text-vitem-500">Kategori *</label>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="flex items-center gap-2 pt-4">
                    ...
                    <label htmlFor="isFeatured" className="text-sm text-vitem-600">Öne Çıkan Ürün</label>
                  </div>
                  <F label="Sıra" name="sortOrder" type="number" defaultValue="0" />
                </div>
